...
클라이언트 UI 아키텍처는 유지보수성과 테스트 용이성을 위해 MVVM 패턴을 적용한다.
...
- FR-3: 사용자는 원격 화면을 실시간으로 볼 수 있어야 하며, 마우스와 키보드 입력으로 원격 PC를 제어할 수 있어야 한다.
...
- NFR-11: 클라이언트 UI 레이어는 MVVM 패턴을 적용하여 View, ViewModel, Model의 책임을 분리해야 한다.
- NFR-12: 개발 및 배포를 위한 빌드 출력 경로는 단순하고 예측 가능하게 유지되어야 하며, 팀이 쉽게 접근할 수 있는 구조로 표준화해야 한다.
...
- 클라이언트 애플리케이션 구조는 MVVM 패턴을 기준으로 설계하여 UI 로직과 비즈니스 로직의 혼합을 방지해야 한다.
- 빌드 아티팩트 경로는 불필요하게 깊거나 복잡하지 않도록 관리해야 하며, 운영 및 테스트 전달 시 혼선을 줄일 수 있어야 한다.
